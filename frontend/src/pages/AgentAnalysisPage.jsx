...
import {
  ...
  getDailyActivity, getProductivityMetrics,
} from '../services/api'
import {
...
export default function AgentAnalysisPage({ user }) {
  ...
  const [prodSelectedAgent, setProdSelectedAgent] = useState(null)
  ...
  const loadProductivity = async (start, end) => {
    setProdLoading(true)
    try {
      const { data } = await getProductivityMetrics({
        start_date: start || prodStartDate,
        end_date: end || prodEndDate,
      })
      setProdData(data)
    } catch (e) {
  ...
  if (view === 'productivity') {
    const AGENT_COLORS = ['#E5A800', '#6366f1', '#16a34a', '#dc2626', '#0ea5e9', '#f97316', '#8b5cf6', '#ec4899', '#14b8a6', '#84cc16']

    // Build comparison chart data (all agents per day)
    const comparisonData = (() => {
      if (!prodData?.agents?.length) return []
      const dayMap = {}
      prodData.agents.forEach((ag, idx) => {
        (ag.daily || []).forEach(d => {
          if (!dayMap[d.date]) dayMap[d.date] = { date: d.date.slice(5) }
          dayMap[d.date][ag.agent_name] = d.tickets_per_hour
        })
      })
      return Object.values(dayMap).sort((a, b) => a.date.localeCompare(b.date))
    })()

    // Individual agent detail data
    const detailAgent = prodData?.agents?.find(a => a.agent_id === prodSelectedAgent)
    const detailData = detailAgent?.daily?.map(d => ({
      date: d.date.slice(5),
      'Tempo Resposta (min)': d.avg_response_time_min,
      'Tempo Resolução (h)': d.avg_resolution_time_h,
      'Tickets/Hora': d.tickets_per_hour,
    })) || []
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-start justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <button onClick={() => setView('overview')}
              ...
              <i className="fas fa-arrow-left text-xs" style={{ color: '#6B7280' }} />
            </button>
            <div>
              <h1 className="text-lg font-bold" style={{ color: '#111827' }}>
                <i className="fas fa-gauge-high mr-2 text-sm" style={{ color: '#B8860B' }} />
                Produtividade da Equipe
              </h1>
              <p className="text-xs mt-0.5" style={{ color: '#6B7280' }}>
                Tempo de resposta, resolução e throughput por agente
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1">
              {[{ label: 'Hoje', key: 'today' }, { label: '7 dias', key: '7d' }, { label: '30 dias', key: '30d' }].map(p => (
            ...
            <div className="flex items-center gap-1">
              <input type="date" value={prodStartDate}
                onChange={e => setProdStartDate(e.target.value)}
                className="px-2 py-1.5 rounded-lg text-xs border" style={{ borderColor: '#D1D5DB', color: '#374151' }} />
              <span className="text-xs" style={{ color: '#9CA3AF' }}>até</span>
              <input type="date" value={prodEndDate}
                onChange={e => setProdEndDate(e.target.value)}
                className="px-2 py-1.5 rounded-lg text-xs border" style={{ borderColor: '#D1D5DB', color: '#374151' }} />
              <button onClick={() => loadProductivity()}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white transition-all hover:shadow-md active:scale-[0.98]"
                style={{ background: '#E5A800' }}>
                <i className="fas fa-search mr-1" />Filtrar
              </button>
            </div>
          </div>
        </div>
        ...
        {prodLoading ? (
          <div className="flex items-center justify-center py-20">
            <i className="fas fa-circle-notch fa-spin text-2xl" style={{ color: '#E5A800' }} />
          </div>
        ) : !prodData ? (
          <div className="text-center py-20" style={{ color: '#9CA3AF' }}>
            <i className="fas fa-gauge-high text-4xl mb-3 block" />
          ...
          <>
            ...
            <div className="glass-card overflow-hidden mb-6">
              <div className="px-5 py-3 border-b" style={{ borderColor: '#F3F4F6' }}>
                <SectionHeader icon="fa-table-list">Metricas por Agente</SectionHeader>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: '#FAFAFA' }}>
                      <th className="text-left px-5 py-3 font-semibold" style={{ color: '#6B7280' }}>Agente</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Tempo Med. Resposta</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Tempo Med. Resolução</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Tickets/Hora</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Total Tickets</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Resolvidos</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}>Horas Trab.</th>
                      <th className="text-center px-4 py-3 font-semibold" style={{ color: '#6B7280' }}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {prodData.agents.map((ag, idx) => {
                      const t = ag.totals
                      const isSelected = prodSelectedAgent === ag.agent_id
                      return (
                        <tr key={ag.agent_id}
                          className="border-t hover:bg-amber-50/30 transition-colors cursor-pointer"
                          style={{ borderColor: '#F3F4F6', background: isSelected ? 'rgba(229,168,0,0.06)' : undefined }}
                          onClick={() => setProdSelectedAgent(isSelected ? null : ag.agent_id)}>
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-2 h-2 rounded-full" style={{ background: AGENT_COLORS[idx % AGENT_COLORS.length] }} />
                              <span className="font-medium" style={{ color: '#111827' }}>{ag.agent_name}</span>
                            </div>
                          </td>
                          <td className="text-center px-4 py-3">
                            <span className="font-mono-carbon font-semibold" style={{ color: t.avg_response_time_min <= 30 ? '#16a34a' : t.avg_response_time_min <= 60 ? '#ca8a04' : '#dc2626' }}>
                              {t.avg_response_time_min}min
                            </span>
                          </td>
                          <td className="text-center px-4 py-3">
                            <span className="font-mono-carbon font-semibold" style={{ color: t.avg_resolution_time_h <= 12 ? '#16a34a' : t.avg_resolution_time_h <= 24 ? '#ca8a04' : '#dc2626' }}>
                              {t.avg_resolution_time_h}h
                            </span>
                          </td>
                          <td className="text-center px-4 py-3">
                            <span className="font-mono-carbon font-bold" style={{ color: '#B8860B' }}>
                              {t.tickets_per_hour}
                            </span>
                          </td>
                          <td className="text-center px-4 py-3 font-mono-carbon" style={{ color: '#374151' }}>{t.total_tickets}</td>
                          <td className="text-center px-4 py-3 font-mono-carbon" style={{ color: '#374151' }}>{t.total_resolved}</td>
                          <td className="text-center px-4 py-3 font-mono-carbon" style={{ color: '#374151' }}>{t.total_hours_worked}h</td>
                          <td className="text-center px-4 py-3">
                            <i className={`fas fa-chevron-${isSelected ? 'up' : 'down'} text-xs`} style={{ color: '#9CA3AF' }} />
                          </td>
                        </tr>
                      )
                    })}
                    {prodData.agents.length === 0 && (
                      <tr><td colSpan={8} className="text-center py-8 text-sm" style={{ color: '#9CA3AF' }}>Nenhum dado no periodo</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            {/* ── Comparison chart: Tickets/Hour per agent per day ── */}
            {comparisonData.length > 0 && (
              <div className="glass-card p-5 mb-6">
                <SectionHeader icon="fa-chart-bar">Tickets/Hora por Dia — Todos os Agentes</SectionHeader>
                <div style={{ height: 320 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={comparisonData} barGap={2} barCategoryGap="15%">
                      ...
                      {prodData.agents.map((ag, idx) => (
                        <Bar key={ag.agent_id} dataKey={ag.agent_name}
            ...
            {/* ── Individual agent detail chart ── */}
            {detailAgent && detailData.length > 0 && (
              <div className="glass-card p-5">
                <SectionHeader icon="fa-chart-line">
                  Detalhe Diario — {detailAgent.agent_name}
                </SectionHeader>
                <div style={{ height: 320 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={detailData}>
                      ...
                      <Tooltip contentStyle={{ borderRadius: 8, border: '1px solid #E5E7EB', fontSize: 12 }} />
                      <Legend wrapperStyle={{ fontSize: 12 }} />
                      <Line yAxisId="left" type="monotone" dataKey="Tempo Resposta (min)"
                        stroke="#6366f1" strokeWidth={2} dot={{ r: 3 }} />
                      <Line yAxisId="left" type="monotone" dataKey="Tempo Resolução (h)"
                        stroke="#dc2626" strokeWidth={2} dot={{ r: 3 }} />
                      <Line yAxisId="right" type="monotone" dataKey="Tickets/Hora"
                        stroke="#E5A800" strokeWidth={2} dot={{ r: 3 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
