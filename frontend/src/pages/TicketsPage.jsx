...
export default function TicketsPage({ user }) {
  ...
  const openAutoAssignModal = () => {
    setSelectedAgentIds(agents.filter(a => a.is_active).map(a => a.id))
  ...
  const handleAutoAssign = async () => {
    ...
    try {
      const ids = selectedAgentIds.length > 0 ? selectedAgentIds : null
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] p-8">
      ...
      {/* Modal: Auto-Assign Agent Selection */}
      {showAutoAssignModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowAutoAssignModal(false)}>
          <div className="bg-[var(--bg-secondary)] rounded-2xl w-full max-w-md border border-[var(--border-color)] shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-[var(--border-color)] flex items-center justify-between">
              ...
              <button onClick={() => setShowAutoAssignModal(false)} className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors">
                <i className="fas fa-times" />
              </button>
            </div>

            <div className="px-6 py-3 border-b border-[var(--border-color)] flex items-center justify-between">
              <span className="text-xs text-[var(--text-tertiary)]">{selectedAgentIds.length} de {agents.filter(a => a.is_active).length} selecionados</span>
              <div className="flex gap-2">
                <button
                  onClick={() => setSelectedAgentIds(agents.filter(a => a.is_active).map(a => a.id))}
            ...
            <div className="px-6 py-3 max-h-80 overflow-y-auto space-y-1">
              {agents.filter(a => a.is_active).map(agent => (
                <label
                  key={agent.id}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-colors hover:bg-[var(--bg-hover)]"
                >
                  <input
                    type="checkbox"
            ...
            <div className="px-6 py-4 border-t border-[var(--border-color)] flex justify-end gap-3">
              ...
              >
                ...
                Distribuir para {selectedAgentIds.length} agente(s)
              </button>
            </div>
          </div>
        </div>
