    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <ChartCard title="Por Categoria">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={categoryData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={40} outerRadius={80} paddingAngle={2} label={false}>
                {categoryData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
              <Tooltip contentStyle={cs.tooltip} formatter={(value, name) => [`${value} tickets`, name]} />
            </PieChart>
          </ResponsiveContainer>
          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 justify-center">
            {categoryData.slice(0, 8).map((d, i) => (
              <span key={d.name} className="flex items-center gap-1 text-[10px]" style={{ color: 'var(--text-secondary)' }}>
