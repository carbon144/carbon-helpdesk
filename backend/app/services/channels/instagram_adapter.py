...
class InstagramAdapter(ChannelAdapter):
    ...
    async def send_interactive(
        ...
        Instagram supports up to 13 quick replies, title max 20 chars.
        """
        if not options:
        ...
        quick_replies = []
        for opt in options[:13]:  # IG max 13 quick replies
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                resp.raise_for_status()
                data = resp.json()
                logger.info("Instagram interactive sent to %s (%d options)", recipient_id, len(options))
                return data
        except httpx.HTTPError as e:
            logger.error("Instagram interactive failed for %s: %s — falling back to text", recipient_id, e)
