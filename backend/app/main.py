...
from app.api import auth, tickets, inboxes, dashboard, kb, slack, gmail, ai, reports, export, ws, tracking, shopify, media, ecommerce, catalog, gamification, rewards, meta, customers, agent_analysis, chat, chatbot, triage, ra_monitor, metricas, ai_agents
...
async def _run_email_fetch_loop():
    ...
    # Subjects generated by our own system — never create tickets from these
    OWN_SYSTEM_SUBJECTS = [
        "chamado #", "ticket #",
        "como foi seu atendimento",
        "tickets que ainda não viu",
        "código para a postagem",
        "carbon smartwatch — ",
        "loja carbon",
    ]

    def _is_own_email(email_data: dict) -> bool:
        """Check if this email was sent by our own system (outbound that looped back)."""
        sender = (email_data.get("from_email") or "").lower().strip()
        support_email = (settings.GMAIL_SUPPORT_EMAIL or "").lower().strip()

        # Primary check: sender matches our support email
        if support_email and sender == support_email:
            return True

        # Also catch common variations (e.g. aliased addresses)
        if support_email and sender.split("@")[-1] == support_email.split("@")[-1]:
            # Same domain — check if it's a system address
            local_part = sender.split("@")[0]
            system_locals = ["atendimento", "suporte", "noreply", "no-reply", "helpdesk", "carbon"]
            if local_part in system_locals:
                return True

        # Check system-generated subject patterns
        subject = (email_data.get("subject") or "").lower()
        for pattern in OWN_SYSTEM_SUBJECTS:
        ...
        try:
            ...
            if emails:
                ...
                for email_data in emails:
                    ...
                    # Skip our own outbound emails (auto-replies, CSAT, notifications that looped back)
                    if _is_own_email(email_data):
                        logger.info(f"[OWN-EMAIL] Skipping own email: {email_data.get('from_email', '?')} — {email_data.get('subject', '?')[:80]}")
                    ...
                    try:
                        async with async_session() as db:
                            ...
                            if existing_ticket:
                                # Determine message type: outbound if from our support email, inbound otherwise
                                support_email = (settings.GMAIL_SUPPORT_EMAIL or "").lower().strip()
                                ...
                                # Only reopen ticket and reset SLA for real customer messages
                                if msg_type == "inbound":
                                ...
                                try:
                                    from app.services.agent_ai_service import get_agent_for_ticket, generate_agent_reply, should_auto_send as _should_auto_send
                                    ai_agent = await get_agent_for_ticket(ticket, db)
                                    if ai_agent:
                                        ticket.ai_agent_id = ai_agent.id
                                        ticket.ai_processing = True
                                        await db.flush()
                                        _order_data = None
                                        try:
                                        ...
                                        agent_result = await generate_agent_reply(
                                            agent=ai_agent,
                                            ticket=ticket,
                                        ...
                                        if agent_result["should_escalate"]:
                                            ...
                                            logger.info(f"AI Agent {ai_agent.name} escalated ticket #{ticket.number}: {agent_result['escalation_reason']}")
                                        elif agent_result["reply_text"]:
                                            ai_agent.total_replies += 1
                                            if await _should_auto_send(ai_agent, agent_result["confidence"]):
                                                from app.services.gmail_service import send_email as _send_email
                                                sent_reply = await asyncio.to_thread(
                                                    ...
                                                    body_text=agent_result["reply_text"],
                                                ...
                                                if sent_reply:
                                                    ...
                                                    _agent_handled = True
                                                    logger.info(f"AI Agent {ai_agent.name} auto-sent reply for ticket #{ticket.number} (confidence={agent_result['confidence']:.2f})")
                                            else:
                                                ticket.ai_draft_text = agent_result["reply_text"]
                                                ticket.ai_draft_confidence = agent_result["confidence"]
                                                existing_tags = list(ticket.tags or [])
                                                existing_tags.append("agent_draft")
                                                ticket.tags = list(set(existing_tags))
                                                _agent_handled = True
                                                logger.info(f"AI Agent {ai_agent.name} drafted reply for ticket #{ticket.number} (confidence={agent_result['confidence']:.2f})")
                                except Exception as e:
                                    logger.warning(f"AI Agent routing skipped in auto-fetch: {e}")
                                finally:
                                    ticket.ai_processing = False
...
async def _run_supervisor_loop():
    """Background task: Carlos supervisor checks pending human actions every 30 min."""
    from app.services.supervisor_service import run_supervisor_check
    sup_logger = logging.getLogger("supervisor")
    await asyncio.sleep(180)  # Wait 3 min for startup
    while True:
        try:
            async with async_session() as session:
                result = await run_supervisor_check(session)
                if result["cobrados"] or result["escalados"]:
                    sup_logger.info(f"Supervisor check: {result['checked']} checked, {result['cobrados']} cobrados, {result['escalados']} escalados")
        except Exception as e:
            sup_logger.error(f"Supervisor check failed: {e}")
        await asyncio.sleep(1800)  # 30 minutes


async def _run_csat_loop():
    """Background task: send CSAT emails 24h after ticket resolution."""
    from datetime import datetime, timezone, timedelta
    from app.models.ticket import Ticket
    from sqlalchemy import select, and_

    csat_logger = logging.getLogger("csat")
    ...
    while True:
        try:
            async with async_session() as db:
                ...
                # Find resolved tickets where CSAT not sent yet, resolved > 24h ago
                result = await db.execute(
                    select(Ticket).where(
                        and_(
                ...
                for ticket in tickets:
                    try:
                        ...
                        success = send_csat_email(ticket)
                        if success:
                            ticket.csat_sent = True
                            sent_count += 1
                    except Exception as e:
                        csat_logger.warning(f"CSAT email failed for ticket #{ticket.number}: {e}")

                if sent_count:
                    await db.commit()
                    csat_logger.info(f"CSAT: {sent_count} emails sent")
        except Exception as e:
            csat_logger.error(f"CSAT loop failed: {e}")
...
async def lifespan(app: FastAPI):
    ...
    # Auto-reseed KB if table is empty (all environments)
    async with async_session() as session:
        from sqlalchemy import func as sa_func
        from app.models.kb_article import KBArticle
        count = await session.execute(select(sa_func.count()).select_from(KBArticle))
        if count.scalar() == 0:
            from app.services.seed_kb import reseed_kb
            await reseed_kb(session)
            logging.getLogger("startup").info("KB articles seeded from kb_real_data")

    # Seed AI agents if empty
    async with async_session() as session:
        from app.services.seed_ai_agents import seed_ai_agents
        await seed_ai_agents(session)
    ...
    supervisor_task = asyncio.create_task(_run_supervisor_loop())
    csat_task = asyncio.create_task(_run_csat_loop())
