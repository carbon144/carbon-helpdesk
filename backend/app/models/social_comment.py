...
class SocialComment(Base):
    ...
    post_id: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)  # Post/media ID
    parent_comment_id: Mapped[str | None] = mapped_column(String(100), nullable=True)  # If it's a reply to another comment
    ...
    post_caption: Mapped[str | None] = mapped_column(Text, nullable=True)  # Caption of the post being commented on

    # AI moderation result
    ai_action: Mapped[str] = mapped_column(String(30), index=True)  # "replied" | "hidden" | "hidden_replied" | "ignored" | "flagged"
    ai_reply: Mapped[str | None] = mapped_column(Text, nullable=True)  # AI's reply text
    ai_sentiment: Mapped[str | None] = mapped_column(String(20), nullable=True)  # "positive" | "neutral" | "negative" | "offensive"
    ai_category: Mapped[str | None] = mapped_column(String(50), nullable=True)  # "elogio" | "duvida" | "reclamacao" | "ofensivo" | "spam" | "mencao" | "outro"
