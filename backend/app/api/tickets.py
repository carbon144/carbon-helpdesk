...
async def list_tickets(
    ...
    if exclude_status:
        ...
        query = query.where(Ticket.status.notin_(excluded))
    if priority:
    ...
    if search:
        # Busca: assunto, número, nome/email do cliente, tracking
        search_filters = [
...
class TicketMergeRequest(BaseModel):
    ...
    source_ticket_id: str | None = None  # backward compat
    target_ticket_id: str


async def _follow_ticket_merge_chain(db: AsyncSession, ticket: Ticket, max_depth: int = 10) -> Ticket:
    """Follow merged_into_id chain to find the real (non-merged) ticket."""
    current = ticket
    visited = {current.id}
    for _ in range(max_depth):
        if current.status != "merged" or not current.merged_into_id:
            return current
        if current.merged_into_id in visited:
            return current
        visited.add(current.merged_into_id)
        result = await db.execute(select(Ticket).where(Ticket.id == current.merged_into_id))
...
async def merge_tickets(
    ...
    """Merge one or more source tickets into target. Follows merge chains automatically."""
    # Resolve source IDs (support both singular and plural)
    raw_source_ids = body.source_ticket_ids or []
    if body.source_ticket_id and body.source_ticket_id not in raw_source_ids:
        raw_source_ids.append(body.source_ticket_id)
    if not raw_source_ids:
        raise HTTPException(status_code=400, detail="Nenhum ticket de origem informado")
    ...
    # If target is merged, follow chain to real target
    if target.status == "merged":
        target = await _follow_ticket_merge_chain(db, target)

    total_moved = 0
    merged_sources = []

    for src_id in raw_source_ids:
        if src_id == target.id:
            continue  # skip if source == target

        src_result = await db.execute(select(Ticket).where(Ticket.id == src_id))
        source = src_result.scalar_one_or_none()
        if not source:
            continue

        # If source is already merged, follow chain to real ticket
        if source.status == "merged":
            source = await _follow_ticket_merge_chain(db, source)

        # After following chain, if source is already the target, skip
        if source.id == target.id:
            continue

        # If already merged (into target), skip
        if source.status == "merged":
            continue

        # 1. Move all messages from source to target (preserve origin)
        ...
        for msg in source_messages:
            moved_ids.append(msg.id)
            if not msg.original_ticket_id:
                msg.original_ticket_id = source.id
            msg.ticket_id = target.id

        # 2. Copy internal notes
        ...
        merged_sources.append({
            ...
            "messages_moved": len(moved_ids),
            "moved_message_ids": moved_ids,
        })

    if not merged_sources:
        raise HTTPException(status_code=400, detail="Nenhum ticket válido para mesclar")

    # 4. Update target
    target.updated_at = datetime.now(timezone.utc)

    # 5. Single audit log with all sources
    db.add(AuditLog(
        ticket_id=target.id,
        user_id=user.id,
        action="ticket_merge",
        details={
            ...
            "total_messages_moved": total_moved,
        },
    ))
    ...
    source_nums = ", ".join(f"#{s['source_ticket_number']}" for s in merged_sources)
    logger.info(
        f"Ticket merge: {source_nums} -> #{target.number} "
        f"({total_moved} messages moved) by {user.name}"
    )
